...
fn toyc_is_ll1() {
    ...
    let toy_c_program: HashSet<Vec<Symbol<token::Token, &'static str>>> =
        [vec!["ToyCProgram'".into(), (Token::Eof).into()]].into();

    let toy_c_program_p: HashSet<Vec<Symbol<token::Token, &'static str>>> =
    ...
    let productions: std::collections::HashMap<&str, HashSet<Vec<Symbol<Token, &str>>>> = [
        ("ToyCProgram", toy_c_program),
        ("ToyCProgram'", toy_c_program_p),
